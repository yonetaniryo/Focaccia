...
class focacciaEllipse(Focaccia):
  ...
  def apply (self, axes_x: float, axes_y: float, angle: float, color_b: float, color_g: float, color_r: float, thickness: int):
    bgr_image = self.toBgr()
    applied_image = bgr_image.copy()
    cv2.ellipse(applied_image, ((self.center[0], self.center[1]), (axes_x, axes_y), angle), (color_b, color_g, color_r), thickness=thickness)
    return self.bgr2rgb(applied_image)
  
  def function (self, axes_x: float, axes_y: float, angle: float, color_b: float, color_g: float, color_r: float, thickness: int):
  ...
  def objective (self, trial):
    ...
    return self.function(axes_x, axes_y, angle, color_b, color_g, color_r, thickness)
